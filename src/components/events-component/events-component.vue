  ...
  <div class="schedule">
    <div class="schedule__container _container">
      <div class="schedule__search">
        <input
          type="text"
          placeholder="Найди свое меропнриятие"
          v-model="searchValue"
          @input="$emit('inputSearch', searchValue)"
        />
      ...
      <div class="schedule__tabs schedule-tabs">
        <nav class="schedule-tabs__items">
          <button
            ...
            v-for="(value, key) in WeekDay"
            :class="{ _active: key.toString() === filters.day }"
            @click="$emit('selectWeekDay', key)"
          >
            {{ value }}
          </button>
        </nav>
        <div class="schedule-tabs__body">
          <div class="schedule-tabs__block _active" id="tab1">
            <event-search
              v-for="event in isShowMore ? events.slice(0, 4) : events"
              :event="event"
            ></event-search>
          </div>
        </div>
        <div class="schedule__footer">
          <router-link to="/events" class="schedule__btn" v-if="events.length > 4 && isShowMore"
            >Показать еще <i class="fa fa-chevron-right"></i>
          </router-link>
